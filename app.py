...
T = {
    ...
    "hard_fact_checking_by_claim": "Analyse épistémique des affirmations",
    "claim": "Affirmation",
    "status": "Statut",
...
def analyze_article(text: str) -> Dict:
    ...
    descriptive_normative_confusion = detect_descriptive_normative_confusion(text)
    petition_analysis = detect_petition_principii(text)
    ...
    result = {
        ...
        "statistical_manipulation_score": statistical_manipulation_analysis["score"],
...
def detect_discourse_type_from_rhetoric(text: str, rhetorical_scores: dict):
    ...
    scores["pamphlétaire"] += rhetorical_scores.get("saturation_rhetorique", 0) * 0.45
    ...
    economic_base = rhetorical_scores.get("economie_politique", 0)
    
    scores["economie_politique"] += economic_base * 2.2
    
    if economic_base > 0:
        scores["economie_politique"] += rhetorical_scores.get("technicite", 0) * 0.35
        scores["economie_politique"] += rhetorical_scores.get("coherence_performative", 0) * 0.20
        scores["economie_politique"] += rhetorical_scores.get("abstraction", 0) * 0.10
    else:
        scores["economie_politique"] = 0
    ...
    scores["journalistique"] += rhetorical_scores.get("reported_discourse_factor", 0) * 1.6
    scores["journalistique"] += rhetorical_scores.get("encyclopedique", 0) * 0.45
    scores["journalistique"] += rhetorical_scores.get("journalistique", 0) * 0.65
    ...
    if (
        ...
        and rhetorical_scores.get("implicite", 0) > 0.4
        and (
            rhetorical_scores.get("attaque", 0) > 0.3
            or rhetorical_scores.get("saturation_rhetorique", 0) > 0.5
        )
    ):
        scores["conspirationniste"] += 0.25
...
def detect_discourse_type(result):
    ...
    if (
        ...
        return (
            "Discours pseudo-analytique / civilisationnel",
            "Le texte adopte une apparence analytique tout en mobilisant des analogies historiques, des arguments de nature ou des causalités fragiles."
        )
...
st.subheader(f"{couleur_r} Crédibilité épistémique apparente : {etiquette_r}")
# Barre épaisse colorée
st.caption(
    "Cette jauge estime la crédibilité épistémique apparente du texte à partir "
    "de son ancrage au réel, de sa cohérence interne, de sa qualité "
    "argumentative, de ses marqueurs de connaissance et de sa révisabilité."
)
st.markdown(f"""
<div style="width:100%; margin-top:10px; margin-bottom:10px;">
    <div style="
...
with js4:
    ...
    with st.popover("ℹ️ Comprendre cette jauge"):
        ...
        st.write(f"Marqueurs détectés : **{len(markers)}**")
        st.write(
            f"Marqueurs de nuance : **{result.get('advanced_deceptive_coherence_nuance_count', 0)}**"
        )
